...
Emailer.prototype.getTransport = function() {
	return nodemailer.createTransport({
		...
		auth: {
			user: sails.config.email.auth.username,
			pass: sails.config.email.auth.password
		}
	});
};

Emailer.prototype.getHTML = function(templateName, data) {
	var templateDir = sails.config.templateDir || path.join(__dirname, '../views/email');
	var templatePath = path.join(templateDir, templateName + '.html');
...
Emailer.prototype.send = function(cb) {
	...
	this.getTransport().sendMail({
		from: '"' + sails.config.email.name + '" <' + sails.config.email.address + '>',
